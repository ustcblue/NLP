...
class LSTMTheano:
    ...
    def __theano_build__(self):
        ...
        grads, updates = self.calc_grad(o_error, params, 16)
        #theano.grad(loss, params)
        
        if len(grads) > 0:
            param_updates = self.adam(o_error, params, grads, learning_rate)
        ...
        # SGD
        self.sgd_step = theano.function([x, y, mask, learning_rate], [], \
    ...
    def calc_grad(self, loss, params, mini_batch_size):
         
        grad = theano.grad(loss, params)
        idx = theano.shared(np.float32(1))
        ...
        for p, g in zip(params, grad):
            sum_g = theano.shared(np.zeros(p.get_value().shape, dtype=theano.config.floatX))
            new_sum_g = sum_g + g

            if idx % mini_batch_size == 0:
                grads.append(new_sum_g / mini_batch_size)
                updates.append((sum_g, np.zeros(g.shape)))
            else:
                updates.append((sum_g, new_sum_g))

        updates.append((idx, idx + 1.))

        return grads, updates

    def adam(self, loss, all_params, all_grads, learning_rate=0.001, b1=0.9, b2=0.999, e=1e-8,
                 gamma=1-1e-8):
        """
        ADAM update rules
